        ...
        .then(data => {
            ...
            data.forEach(place => {
                ...
                article.innerHTML = `
                    ...
                    <div class="reviews">
                        <h2>Reviews <span data-place-id="${place.id}">show</span></h2>
                        <ul class="review_list"></ul>
                    </div>`;
                placesSection.appendChild(article);
            });

            document.querySelectorAll('.reviews span').forEach(span => {
                span.addEventListener('click', function () {
                    const placeId = this.dataset.placeId;
                    const reviewList = this.nextElementSibling;
                        ...
                        fetch(`/api/v1/places/${placeId}/reviews`)
                            ...
                            .then(reviews => {
                                ...
                                reviews.forEach(review => {
                                    ...
                                    li.textContent = `${review.user.first_name} ${review.user.last_name}: ${review.text}`;
